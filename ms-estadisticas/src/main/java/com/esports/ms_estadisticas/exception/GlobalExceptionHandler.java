...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(EstadisticaYaExisteException.class)
    public ResponseEntity<ErrorResponse> handleEstadisticaYaExiste(EstadisticaYaExisteException ex,
            HttpServletRequest rq) {
        log.warn("La Estadística ya existe: {} | URI: {}", ex.getMessage(), rq.getRequestURI());

        ErrorResponse error = ErrorResponse.builder()
                .timestamp(LocalDateTime.now())
                .status(HttpStatus.CONFLICT.value())
                .error("La estadística ya existe")
    ...
    public ResponseEntity<ErrorResponse> handleJugadorNoEncontrado(JugadorNoEncontradoException ex,
            HttpServletRequest rq) {
        log.warn("El Jugador no ha sido encontrado: {} | URI: {}", ex.getMessage(), rq.getRequestURI());

        ErrorResponse error = ErrorResponse.builder()
                .timestamp(LocalDateTime.now())
                .status(HttpStatus.NOT_FOUND.value())
                .error("El jugador no se encontró")
                .mensaje(ex.getMessage())
                .path(rq.getRequestURI())
                .build();
    ...
    public ResponseEntity<ErrorResponse> handlePartidaNoEncontrada(PartidaNoEncontradaException ex,
            HttpServletRequest rq) {
        log.warn("La Partida no ha sido encontrada: {} | URI: {}", ex.getMessage(), rq.getRequestURI());

        ErrorResponse error = ErrorResponse.builder()
                .timestamp(LocalDateTime.now())
                .status(HttpStatus.NOT_FOUND.value())
                .error("La partida no se encontró")
                .mensaje(ex.getMessage())
                .path(rq.getRequestURI())
                .build();
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(error);
