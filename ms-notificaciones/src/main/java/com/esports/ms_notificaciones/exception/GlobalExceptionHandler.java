...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleJugadorNoExiste(JugadorNoExisteException ex, HttpServletRequest rq) {
        log.warn("El jugador no existe: {} | URI: {}", ex.getMessage(), rq.getRequestURI());
    ...
    @ExceptionHandler(NotificacionNoExisteException.class)
    public ResponseEntity<ErrorResponse> handleNotificacionNoExiste(NotificacionNoExisteException ex,
            HttpServletRequest rq) {
        log.warn("La notificación no existe: {} | URI: {}", ex.getMessage(), rq.getRequestURI());
    ...
    public ResponseEntity<ErrorResponse> handleNotificacionYaLeida(NotificacionYaLeidaException ex,
            HttpServletRequest rq) {
        log.warn("Notificación ya leída: {} | URI: {}", ex.getMessage(), rq.getRequestURI());

        ErrorResponse error = ErrorResponse.builder()
                .timestamp(LocalDateTime.now())
                .status(HttpStatus.BAD_REQUEST.value())
                .error("Estado de notificación inválido")
                .mensaje(ex.getMessage())
                .path(rq.getRequestURI())
                .build();
